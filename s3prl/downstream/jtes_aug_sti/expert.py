...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, expdir, **kwargs):
        ...
        self.fold = self.datarc.get(
            'test_fold') or kwargs.get("downstream_variant")
        ...
        train_path = os.path.join(
            DATA_ROOT, 'meta_data', self.fold.replace('fold', 'sti'),
            'train_meta_data.json')
        print(f'[Expert] - Training path: {train_path}')

        test_path = os.path.join(
            DATA_ROOT, 'meta_data', self.fold.replace('fold', 'sti'), 'test_meta_data.json')
    ...
    def forward(self, mode, features, labels, filenames, records, **kwargs):
        ...
        features_len = torch.IntTensor(
            [len(feat) for feat in features]).to(device=device)
        ...
        records["predict"] += [self.test_dataset.idx2emotion[idx]
                               for idx in predicted_classid.cpu().tolist()]
        records["truth"] += [self.test_dataset.idx2emotion[idx]
                             for idx in labels.cpu().tolist()]
        ...
        for key in ["acc", "loss"]:
            ...
            with open(Path(self.expdir) / "log.log", 'a') as f:
                if key == 'acc':
                    ...
                    if mode == 'test' and average > self.best_score:
