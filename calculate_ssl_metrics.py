...
def run_calc_metric(result_dir):
    ...
    try:
        # Remove the s3prl/ prefix for the command since we'll run from s3prl directory
        relative_result_dir = result_dir.replace("s3prl/", "")
        cmd = [sys.executable, "calc_metric.py", relative_result_dir]
        result = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
            cwd="/home/bagus/github/s3prl-ser/s3prl",
        )
...
def parse_metric_output(output):
    ...
    for line in lines:
        if "WA:" in line or "Weighted Accuracy:" in line:
            try:
                wa = float(line.split(":")[-1].strip().replace("%", ""))
            except (ValueError, IndexError):
                pass
        elif "UA:" in line or "Unweighted Accuracy:" in line:
            try:
                ua = float(line.split(":")[-1].strip().replace("%", ""))
            except (ValueError, IndexError):
                pass
...
def main():
    ...
    for model_name, model_pattern in ssl_models.items():
        ...
        for fold in range(1, 6):  # folds 1-5
            result_dir = f"s3prl/result/downstream/emofilm-cv-fold_{fold}.csv-{model_pattern}-128"
